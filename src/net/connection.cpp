...
void Connection::deleteConnection()
{
	...
	try
	{
		boost::asio::post(m_service, boost::bind(&Connection::onStop, this));
	}
	catch(std::exception& e)
	{
		if(m_logError)
...
void Connection::accept()
{
	...
	{
		++m_pendingRead;
		m_readTimer.expires_after(std::chrono::seconds((int)Connection::readTimeout));
...
void Connection::internalSend(OutputMessage_ptr msg)
{
	...
	{
		++m_pendingWrite;
		m_writeTimer.expires_after(std::chrono::seconds((int)Connection::writeTimeout));
